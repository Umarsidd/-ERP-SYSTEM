  ...
  return (
    <div className="container mx-auto p-4 sm:p-6 space-y-4 sm:space-y-6">
      ...
      <HomeHeader
        ...
        setViewMode={null}
        isMobile={null}
        title={isRTL ? "قائمة الفروع" : "Branches"}
        description={isRTL ? "عرض وإدارة الفروع" : "View and manage branch"}
        other={isRTL ? " فرع جديد" : "New Branch"}
        href={"/branches/new"}
        sectionName={"Branches"}
        pageName={"BranchManagement"}
      />
      ...
      >
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-semibold">
            {isRTL ? "الفروع" : "Branches"} ({users.length})
          </h2>
        </div>
                ...
                <Card className="p-4 hover:shadow-lg transition-shadow">
                  <div className="flex items-start justify-between gap-3">
                      ...
                      <div className="mt-2 flex items-center gap-3">
                        <div className="text-sm">
                          <div className="font-medium">
                            {JSON.parse(c.main)?.phone}
                          </div>
                          <div className="text-muted-foreground text-xs">
                            {JSON.parse(c.main)?.email}
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex flex-col items-end gap-2">
                      ...
                      {/* <Button
                        variant="link"
                        onClick={() => {
                          handleView(c, navigate, `/branches/contacts/${c.id}`);
                        }}
                        //  to={`/branches/view/${c.id}`}
                  ...
                  <div className="mt-3 border-t pt-3 flex items-center justify-between">
                    <div className="text-sm text-muted-foreground">
                      {JSON.parse(c.main)?.city}
                    </div>
                    <div className="flex items-center gap-2">
                      <Button
                        variant="link"
                        onClick={() => {
                          handleView(c, navigate, `/branches/view/${c.id}`);
                        }}
                        //  to={`/branches/view/${c.id}`}
                        className="text-sm text-primary"
                      >
        ...
        {users.length === 0 && !isLoading && (
          <EmptyState
            clearAllFilters={clearFilters}
            title={isRTL ? "لم يتم العثور على فروع" : "No branches found"}
            description={
              isRTL
                ? "ابدأ بإنشاء فرع جديد"
