...
import { Copy, Edit, FileText, Printer, Trash2, Users } from "lucide-react";
...
export default function Branch() {
  ...
  useEffect(() => {
    ...
    console.log(JSON.parse(customer.main));
  }, []);
    ...
    return (
      <div className="container mx-auto p-4 sm:p-6">
        <div className="text-center text-muted-foreground">
          {isRTL ? "الفرع غير موجود" : "Branch Not Found"}
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="container mx-auto p-4 sm:p-6 space-y-4 sm:space-y-6">
        ...
        <div className="flex items-center space-x-4 rtl:space-x-reverse">
          ...
          <div>
            <h1 className="text-2xl font-bold text-foreground">
              {isRTL ? "عرض الفرع" : "Branch Details"}
            </h1>
            <p className="text-muted-foreground mt-1">
              {isRTL ? "تفاصيل ومعلومات الفرع" : "Branch full details"}
            </p>
          </div>
        </div>
          ...
          <div className="flex items-center gap-3">
            ...
            {(JSON.parse(localStorage.getItem("subRole") || "null") as any)
              ...
              <Button
                variant="outline"
                size="sm"
                onClick={() => handleEdit(customer, navigate, `/branches/edit`)}
        ...
        >
          <div className="mt-4 grid grid-cols-1 sm:grid-cols-4 gap-3">
            <div>
              ...
              <div className="font-medium">{customer.elementNumber}</div>
            </div>
            ...
            <div>
              ...
              <div className="font-medium">
                {JSON.parse(customer.main)?.phone}
              </div>
            </div>

            <div>
              <div className="text-xs text-muted-foreground">
                {isRTL ? "الهاتف البديل" : "Alt Phone"}
              </div>
              <div className="font-medium">
                {JSON.parse(customer.main)?.altPhone}
              </div>
            </div>
            ...
            <div>
              ...
              <div className="font-medium">
                {JSON.parse(customer.main)?.email}
              </div>
            </div>
      ...
      {JSON.parse(customer.main)?.usersIDS?.length >= 1 && (
        <motion.div
              ...
              {JSON.parse(customer.main)?.usersIDS?.length >= 1 && (
                <div className="mt- mb-">
                  ...
                  <div className="mt- grid grid-cols-1 sm:grid-cols-4 gap-1">
                    {JSON.parse(customer.main)?.usersIDS.map(
                      (ct, idx) =>
                        ct.name !== "" && (
      ...
      {/* Notes */}
      {JSON.parse(customer.main).notes && (
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ delay: 0.5 }}
