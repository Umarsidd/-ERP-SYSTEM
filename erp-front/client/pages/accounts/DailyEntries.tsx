  ...
  return (
    <div className="container mx-auto p-4 sm:p-6 space-y-4 sm:space-y-6">
      ...
      <HomeHeader
        ...
        description={
          isRTL ? "إدارة ومتابعة الحسابات" : "Manage and track accounts"
        }
        other={isRTL ? "قيد جديد" : "New Account"}
      ...
      >
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-semibold">
            {isRTL ? "الحسابات" : "Accounts"} ({mainData.length})
          </h2>
        </div>
        ...
        {isLoading ? (
          ...
          <SecondaryList
            type="accounts"
            data={mainData}
            titleLink={"daily-entries"}
            title={isRTL ? "الحسابات" : "Accounts"}
            setIsRefreshing={setIsRefreshing}
            sectionName={"Accounts"}
            pageName={"editAndDeleteAllEntry"}
            pageName2={"editAndDeleteHisEntry"}
          />
        )}
        ...
        {mainData.length === 0 && !isLoading && (
          <EmptyState
            clearAllFilters={clearFilters}
            title={isRTL ? "لم يتم العثور على حسابات" : "No Accounts found"}
            description={isRTL ? "" : ""}
