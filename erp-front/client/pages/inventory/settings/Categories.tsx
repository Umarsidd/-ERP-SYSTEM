...
export default function Categories() {
    ...
    const handleSave = async () => {
        if (formData.name) {
            ...
            try {
                const categoryData = {
                    id: editingId || `category_${Date.now()}`,
                    name: formData.name,
                ...
                let newCategories = [];
                if (editingId) {
                    newCategories = categories.map(c => c.id === editingId ? categoryData : c);
                } else {
            ...
            await Swal.fire({
                ...
                text: isRTL ? "يرجى إدخال اسم الفئة" : "Please enter category name",
            });
        }
    };
    ...
    const parentCategories = categories.filter(c => !c.parentId && c.id !== editingId);
            ...
            >
                <div>
                    ...
                    <p className="text-muted-foreground mt-1">
                        {isRTL
                            ? "إدارة تصنيفات وفئات المنتجات"
                            : "Manage product classifications and categories"}
                    </p>
                </div>
                {!showForm && (
                ...
                <Card className="p-6 border-2 border-primary/20">
                    ...
                    >
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                            ...
                            <div>
                                <label className="block text-sm font-medium mb-2">
                                    {isRTL ? "الفئة الرئيسية (اختياري)" : "Main Category (Optional)"}
                                </label>
                                <select
                                ...
                                >
                                    <option value="">{isRTL ? "بدون فئة رئيسية" : "No Main Category"}</option>
                            ...
                            <div className="md:col-span-2">
                                ...
                                <textarea
                                    ...
                                    className="w-full px-3 py-2 border border-border rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent"
                                    placeholder={isRTL ? "وصف للفئة" : "Category description"}
                                    rows={3}
                                />
                            </div>
                        </div>
