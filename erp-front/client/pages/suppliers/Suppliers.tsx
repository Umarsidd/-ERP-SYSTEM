...
const SuppliersManagement: React.FC = () => {
  ...
  const [suppliers, setSupplierss] = useState([]);
  ...
  return (
    <div className="container mx-auto p-4 sm:p-6 space-y-4 sm:space-y-6">
      ...
      <HomeHeader
        ...
        setViewMode={null}
        isMobile={null}
        title={isRTL ? "قائمة الموردين" : "Suppliers"}
        description={
          isRTL
            ? "عرض وإدارة سجلات الموردين، البحث والتصفية حسب الحالة والنوع"
            : "View and manage supplier records, search and filter by status and type"
        }
        other={isRTL ? "مورد جديد" : "New Suppliers"}
        href={"/suppliers/new"}
        sectionName={"Purchases"}
        pageName={"addNewSuppliers"}
      />
      ...
      >
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-semibold">
            {isRTL ? "الموردين" : "Suppliers"} ({suppliers.length})
          </h2>
        </div>
                ...
                {c.name.trim() !== "" && (
                  <Card className="p-4 hover:shadow-lg transition-shadow">
                    <div className="flex items-start justify-between gap-3">
                      <div className="sm:w-20">
                        ...
                        <div className="mt-2 flex items-center flex-wrap gap-3">
                          <div className="text-sm">
                            ...
                            <p className="text-muted-foreground text-xs text-wrap break-all">
                              {c.email}
                            </p>
                          </div>
                        </div>
                      ...
                      <div className="flex flex-col items-end gap-2 px-">
                        <Badge
                          variant={
                            c.status === "Active"
                              ? "default"
                        ...
                        <Button
                          ...
                          onClick={() => {
                            handleView(
                              c,
                              navigate,
                              `/suppliers/contacts/${c.id}`,
                            );
                          }}
                          //  to={`/suppliers/view/${c.id}`}
                        ...
                        {/* <Link
                        to={`/suppliers/contacts?supplier=${c.id}`}
                        className="text-sm text-primary"
                      >
                        {isRTL ? "جهات الاتصال" : "Contacts"}
                      </Link> */}
                    ...
                    <div className="mt-3 border-t pt-3 flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        <Button
                          variant="link"
                          onClick={() => {
                            handleView(c, navigate, `/suppliers/view/${c.id}`);
                          }}
                          //  to={`/suppliers/view/${c.id}`}
                          className="text-sm text-primary"
                        >
        ...
        {suppliers.length === 0 && !isLoading && (
          <EmptyState
            clearAllFilters={clearFilters}
            title={isRTL ? "لم يتم العثور على موردين" : "No suppliers found"}
            description={
              isRTL
                ? "ابدأ بإنشاء مورد جديد"
                : "Get started by creating a new suppliers"
            }
