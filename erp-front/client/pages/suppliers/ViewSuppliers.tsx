...
import { Copy, Edit, Plus, Printer, Trash2, X } from "lucide-react";
import { handleCopy, handleEdit, handlePayment, mainFilter } from "@/lib/function";
...
import LightweightDialog, { LightweightDialogContent, LightweightDialogHeader } from "@/components/ui/lightweight-dialog";
...
export default function SuppliersView() {
  ...
  const [supplier, setSuppliers] = useState<any>(null);
  ...
  var tableNameArray = [
    { name: "purchase_invoices", list: setInvoices, list2: setInvoiceNum },
    { name: "purchase_return", list: setReturns, list2: setReturnNum },
    {
      name: "purchase_credit_notices",
      list: setCreditNotes,
      list2: setCreditNoteNum,
    },
    { name: "purchase_payment", list: setPayments, list2: setPaymentNum },
  ];
  ...
  useEffect(() => {
    ...
    //        console.log("updateCustomerMetayyyyy", supplier);
  }, [showAddModal]);
  ...
  }, [supplier?.id, advancedFilters, sort]);
    ...
    return (
      <div className="container mx-auto p-4 sm:p-6">
        <div className="text-center text-muted-foreground">
          {isRTL ? "المورد غير موجود" : "Suppliers Not Found"}
        </div>
      </div>
    );
  }
      ...
      >
        <div className="flex items-center space-x-4 rtl:space-x-reverse">
          ...
          <div>
            <h1 className="text-2xl font-bold text-foreground">
              {isRTL ? "عرض المورد" : "Suppliers Details"}
            </h1>
            <p className="text-muted-foreground mt-1">
              {isRTL
                ? "تفاصيل المورد ومعلومات الاتصال"
                : "Suppliers full details and contact information"}
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
          {((JSON.parse(localStorage.getItem("subRole") || "null") as any)
            ?.Purchases?.editAndDeleteAllSuppliers !== false ||
            ((JSON.parse(localStorage.getItem("subRole") || "null") as any)
              ?.Purchases?.editAndDeleteHisSuppliers === true &&
              JSON.parse(
                CryptoJS.AES.decrypt(
                  localStorage.getItem("user"),
                  import.meta.env.VITE_SECRET,
                ).toString(CryptoJS.enc.Utf8),
              )?.user?.id === JSON.parse(supplier.createdBy).id)) && (
              <Button
                variant="outline"
                size="sm"
              ...
              >
                <span className=" sm:inline">
                  {JSON.parse(supplier.main)?.openingBalance > 0
                    ? isRTL
              ...
              <Button
                ...
                onClick={() => {
                  handleCopy(
                    isRTL ? "نسخ " : "Copy ",
                    isRTL
                      ? `هل تريد نسخ ${supplier.elementNumber}؟`
                      : `Do you want to copy ${supplier.elementNumber}?`,
                    supplier,
                    isRTL,
                    navigate,
                    "/suppliers/new",
              ...
              <Button
                ...
                onClick={async () => {
                  ...
                  var res = await authApi.delete(
                    isRTL ? "حذف" : "Delete",
                    isRTL
                      ? `هل أنت متأكد من حذف ${supplier.elementNumber}؟ لا يمكن التراجع عن هذا الإجراء.`
                      : `Are you sure you want to delete ${supplier.elementNumber}? This action cannot be undone.`,
                    supplier.id,
                    isRTL,
                  );
          ...
          {((JSON.parse(localStorage.getItem("subRole") || "null") as any)
            ...
            ((JSON.parse(localStorage.getItem("subRole") || "null") as any)
              ...
              <Button
                variant="outline"
                size="sm"
                onClick={() => handleEdit(supplier, navigate, `/suppliers/edit`)}
              ...
              tabs={[
                {
                  ...
                  content: (
                    ...
                    >
                      <div className="mt-4 grid grid-cols-1 sm:grid-cols-4 gap-3">
                        ...
                        <div>
                          ...
                          <div className="font-medium">
                            {" "}
                            {JSON.parse(supplier.main)?.company ||
                        ...
                        <div>
                          ...
                          <div className="font-medium">
                            {JSON.parse(supplier.main)?.vatNumber || "-"}
                          </div>
                        </div>
                        ...
                        <div>
                          ...
                          <div className="font-medium">
                            {JSON.parse(supplier.main)?.address}
                          </div>
                        </div>
                        ...
                        <div>
                          ...
                          <div className="font-medium">{supplier.status}</div>
                        </div>
                        ...
                        <div>
                          ...
                          <div className="font-medium">
                            {JSON.parse(supplier.main)?.state}
                          </div>
                        </div>
                        ...
                        <div>
                          ...
                          <div className="font-medium">
                            {JSON.parse(supplier.main)?.country}
                          </div>
                        </div>
                      </div>

                      {JSON.parse(supplier.main)?.contacts?.length >= 1 && (
                        <div className="mt-8 mb-2">
                          <div className="text-xs text-muted-foreground">
                            {isRTL ? "جهات الاتصال" : "Contacts"}
                          </div>
                          <div className="mt-2 grid grid-cols-1 sm:grid-cols-4 gap-1">
                            {JSON.parse(supplier.main)?.contacts?.map(
                              (ct, idx) =>
                                ct.name !== "" && (
                                  <div
                                    key={idx}
                      ...
                      <div className="mt-8 mb-2">
                        <DisplayImages
                          data={JSON.parse(supplier.attachments)?.images}
                        />
                      </div>
                    </motion.div>
                  ),
