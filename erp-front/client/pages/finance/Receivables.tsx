...
const Receivables: React.FC = () => {
  ...
  const { summary, loading: summaryLoading } =
    useBulkSummary("receivables", advancedFilters, sort);
  ...
  return (
    <div className="container mx-auto p-4 sm:p-6 space-y-4 sm:space-y-6">
      ...
      <HomeHeader
        ...
        setViewMode={null}
        isMobile={null}
        title={isRTL ? "سندات القبض" : "Receipts"}
        description={
          isRTL ? "إدارة وتتبع سندات القبض" : "Manage and track Receipts"
        }
        other={isRTL ? "سند قبض" : "Receipts Voucher"}
        href={"/finance/receivables/create"}
        sectionName={"Finance"}
        pageName={"addingReceipts"}
      />
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <Card className="bg-primary text-white border-none p-4 relative">
          ...
          <h2 className="text-3xl font-bold mt-2">
{summary.last365.toLocaleString()}          </h2>
        </Card>
      </div>
      <Filters
        title={"Receivables"}
      ...
      >
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-semibold">
            {isRTL ? "سندات القبض" : "Receipts"} ({orders.length})
          </h2>
        </div>
        ...
        {isLoading ? (
          ...
          <SecondaryList
            ...
            title={isRTL ? "سندات القبض" : "Receipts"}
            sectionName={"Finance"}
            pageName={"editAndDeleteAllReceipts"}
            pageName2={"editAndDeleteHisReceipts"}
          />
        )}
        <Pagination
        ...
        {orders.length === 0 && !isLoading && (
          <EmptyState
            clearAllFilters={clearFilters}
            title={isRTL ? "لم يتم العثور على سند قبض" : "No Receipts found"}
            description={
              isRTL
                ? "ابدأ بإنشاء سند قبض جديد"
