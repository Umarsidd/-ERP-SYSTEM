...
export default function CreateReceipts() {
  ...
  const [initialValues, setInitialValues] = useState({
    elementNumber: generateNumber("RES"),
    customerId: "",
    customer: {
      id: "",
      name: "",
    ...
    status: "Approved",
    paymentAccountId: "",
    paymentAccount: null as AccountNode | null, // The account receiving money (Bank/Cash)
    sourceAccountId: "",
    sourceAccount: null as AccountNode | null, // The account money comes from (Customer/Other)
  ...
  const schema = Yup.object().shape({
    ...
    // paymentAccountId: Yup.string().required(isRTL ? "حساب الخزينة/البنك مطلوب" : "Treasury/Bank account required"),
  });
  ...
  const handleSubmit = async (values: any) => {
    ...
    try {
      ...
      if (res.result === false) {
        ...
        // Logic for Account Entry (Double Entry) if needed
          ...
          <Form className="space-y-4">
              ...
              <div className="flex items-center space-x-4 rtl:space-x-reverse mt-12 sm:mt-0">
                ...
                <div>
                  <h1 className="text-2xl font-bold">
                    {isRTL ? "إضافة / تحرير المقبوضات" : "Add / Edit Receipts"}
                  </h1>
                  <p className="text-muted-foreground text-sm sm:text-base">
                    {isRTL
                      ? "قم بإضافة إيصال جديد إلى سجلاتك أو تعديل إيصال موجود."
                      : "Add a new receipt to your records or edit an existing receipt."}
                  </p>
                </div>
              </div>
            ...
            <Card className="p-6 bg-background shadow-xs border border-border/60">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-6">
                ...
                {/* Receipt Number */}
                <div className="space-y-2">
                  <Label className="text-sm font-medium">{isRTL ? "رقم الإيصال" : "Receipt Number"}</Label>
                  <Field
                    name="elementNumber"
                    disabled
                ...
                {/* Issue Date */}
                <div className="space-y-2">
                  <Label className="text-sm font-medium">{isRTL ? "تاريخ الإيصال" : "Receipt Date"}</Label>
                  <Field
                    name="issueDate"
                    type="date"
                    className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div className="space-y-2">
                  <Label className="text-sm font-medium">{isRTL ? "حساب الإيداع (إلى)" : "Deposit Account (To)"}</Label>
                  <AccountGuideDropdown
                    ...
                    placeholder={isRTL ? "اختر حساب الإيداع..." : "Select deposit account..."}
                    filterByCategory="1" // Assets (Bank/Cash)
                    className="w-full"
                  />
                  <p className="text-[11px] text-muted-foreground">
                    {isRTL ? "الحساب الذي تم إيداع المبلغ فيه" : "The account where money was deposited"}
                  </p>
                  <ErrorMessage
                ...
                <div className="space-y-2">
                  <Label className="text-sm font-medium">{isRTL ? "الحساب (من)" : "Account (Source)"}</Label>
                  <AccountGuideDropdown
                    accounts={accounts}
                  ...
                  />
                  <p className="text-[11px] text-muted-foreground">
                    {isRTL ? "الحساب الذي استلمنا منه المال" : "The account we received money from"}
                  </p>
                </div>
              </div>
