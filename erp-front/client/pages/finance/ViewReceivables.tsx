...
import { printUnifiedReceivableReceipt } from "@/utils/receiveablePrintPdf";

const ViewReceivables: React.FC = () => {
  const { id } = useParams();
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-4 sm:space-y-0 mb-6 sm:mb-8">
        <div className="flex items-center space-x-4 rtl:space-x-reverse">
          ...
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold text-foreground">
              {isRTL ? "تفاصيل سندات القبض" : "Receivables Details"}
            </h1>
        ...
        <div className="flex items-center space-x-3 rtl:space-x-reverse">
          ...
          <Button
            ...
            onClick={() => {
              handleCopy(
                isRTL ? "نسخ  " : "Copy ",
                isRTL
                  ? `هل تريد نسخ   ${quotation.elementNumber}؟`
                  : `Do you want to copy  ${quotation.elementNumber}?`,
                quotation,
                isRTL,
                navigate,
          ...
          {((JSON.parse(localStorage.getItem("subRole") || "null") as any)
            ?.Finance?.editAndDeleteAllReceipts !== false ||
            ((JSON.parse(localStorage.getItem("subRole") || "null") as any)
              ?.Finance?.editAndDeleteHisReceipts === true &&
              JSON.parse(
              ...
              <Button
                ...
                size="sm"
                onClick={() =>
                  handleEdit(quotation, navigate, `/finance/receivables/edit`)
                }
