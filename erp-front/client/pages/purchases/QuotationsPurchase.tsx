...
const QuotationsPurchase: React.FC = () => {
    ...
    try {
      ...
      if (
        (JSON.parse(localStorage.getItem("subRole") || "null") as any)
          ?.Purchases?.viewHisPriceQuotes === true
      ) {
        filter.push({
          field: "createdBy",
          operator: "=",
  ...
  return (
    <div className="container mx-auto p-4 sm:p-6 space-y-4 sm:space-y-6">
      ...
      <HomeHeader
        ...
        setViewMode={null}
        isMobile={null}
        title={isRTL ? " عروض اسعار المشتريات" : "Purchase Quotations"}
        description={
          isRTL
            ? "إدارة وتتبع عروض الأسعار والاقتراحات التجارية"
      ...
      >
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-semibold">
            {isRTL ? " عروض اسعار المشتريات" : "Purchase Quotations"} (
            {quotations.length})
          </h2>
        </div>
        ...
        {isLoading ? (
          ...
          <SecondaryList
            ...
            data={quotations}
            titleLink={"quotations"}
            title={isRTL ? "عرض الاسعار" : "QUOTATION"}
            sectionName={"Purchases"}
            pageName={"deletingAndEditingAllPriceQuotes"}
            pageName2={"deletingAndEditingHisPriceQuotes"}
          />
        )}
        <Pagination
        ...
        {quotations.length === 0 && !isLoading && (
          <EmptyState
            clearAllFilters={clearFilters}
            title={
              isRTL ? "لم يتم العثور على عروض أسعار" : "No quotations found"
            }
            description={
              isRTL
...
export default QuotationsPurchase;
