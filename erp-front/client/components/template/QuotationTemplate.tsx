        ...
        <div className="flex flex-col space-y-4 mb-6 sm:flex-row sm:justify-between sm:items-start sm:space-y-0 sm:mb-8 invoice-header">
          ...
          <div className="text-left sm:text-right rtl:text-right sm:rtl:text-left">
            ...
            <div className="space-y-2">
              ...
              <div
                className={`inline-flex items-center space-x-2 rtl:space-x-reverse px-3 py-1 rounded-full text-xs sm:text-sm font-medium border ${getStatusColor(data.status)}`}
              >
          ...
          <div>
            ...
            <div className="space-y-2 text-gray-600 text-sm sm:text-base">
              ...
              <div className="flex items-center space-x-2 rtl:space-x-reverse">
                ...
                <span className="break-all">{data.customer.email}</span>
              </div>
              <div className="flex items-center space-x-2 rtl:space-x-reverse">
                <Phone className="w-3 h-3 sm:w-4 sm:h-4 flex-shrink-0" />
                <span className="break-words">{data.customer.phone}</span>
              </div>
            </div>
          </div>
            ...
            <div className="space-y-2 text-gray-600 text-sm sm:text-base">
              <div className="flex justify-between items-start">
                ...
                <span className="font-medium text-right">
                  {new Date(
                    data.date || data.issueDate || new Date(),
                  ).toLocaleDateString()}
                </span>
              </div>
              {/* <div className="flex justify-between items-start">
              ...
              {(data.salesRep || data.salesPerson) && (
                <div className="flex justify-between items-start">
                  <span className="flex-shrink-0">
              ...
              {/* {(data.paymentTerm || data.paymentMethod) && (
                <div className="flex justify-between items-start">
                  <span className="flex-shrink-0">
          ...
          <div className="hidden sm:block overflow-x-auto">
            <table className="w-full border border-gray-300 text-sm">
              <thead className="bg-gray-50">
                <tr>
                  <th className="border border-gray-300 px-3 py-3 text-left rtl:text-right font-semibold lg:px-4">
                    {isRTL ? "الاسم" : "Name"}
                  </th>
                  <th className="border border-gray-300 px-3 py-3 text-center font-semibold w-20 lg:px-4">
                    {isRTL ? "الكمية" : "Qty"}
                  </th>
                  <th className="border border-gray-300 px-3 py-3 text-right rtl:text-left font-semibold w-24 lg:px-4">
                    {isRTL ? "السعر" : "Price"}
                  </th>
                  <th className="border border-gray-300 px-3 py-3 text-center font-semibold w-20 lg:px-4">
                    {isRTL ? "خصم" : "Discount"}
              ...
              <tbody>
                {data.items.map((item, index) => (
                  ...
                  >
                    <td className="border border-gray-300 px-3 py-3 lg:px-4">
                      <div>
                    ...
                    <td className="border border-gray-300 px-3 py-3 text-right rtl:text-left lg:px-4">
                      {formatAmount(
                        convertAmount(
                          item.unitPrice,
                          localStorage.getItem("selectedCurrency") ??
                    ...
                    <td className="border border-gray-300 px-3 py-3 text-right rtl:text-left font-semibold lg:px-4">
                      {formatAmount(
                        convertAmount(
                          item.total || calculateItemTotal(item),
                          localStorage.getItem("selectedCurrency") ??
        ...
        <div className="flex justify-end mb-6 sm:mb-8">
          <div className="w-full max-w-sm">
            <div className="space-y-2 text-sm sm:text-base">
              ...
              {totals.discount > 0 && (
                <div className="flex justify-between text-red-600">
                  <span>{isRTL ? "خصم العرض:" : "Quotation Discount:"}</span>
                  <span className="font-medium">
                    -
                    {formatAmount(
                      convertAmount(
                ...
                <>
                  ...
                  <div className="flex justify-between text-lg font-bold text-green-600">
                    ...
                    <span>
                      {formatAmount(
                        convertAmount(
                          totals.total - data.depositAmount,
                          localStorage.getItem("selectedCurrency") ??
                            selectedCurrency,
                        ),
                        (JSON.parse(data?.currency)?.symbol ||
          ...
          <div className="mb-6 p-4 bg-gray-50 rounded-lg">
            ...
            <div className="text-xs sm:text-sm text-gray-600">
              ...
              <p>
                ...
                {isRTL
                  ? data.shippingAddressAr || data.shippingAddress
                  : data.shippingAddress}
              </p>
            </div>
          </div>
                ...
                <div
                  dangerouslySetInnerHTML={{
                    __html: isRTL
                      ? data.termsAr ||
                        data.terms ||
                        "الدفع مستحق خلال 30 يوماً من تاريخ العرض. قد تُطبق رسوم تأخير."
                      : data.terms ||
                        data.termsAr ||
                        "Payment is due within 30 days of quotation date. Late fees may apply.",
                  }}
                />
              </div>
            </div>
