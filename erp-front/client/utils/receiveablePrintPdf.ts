...
export const printUnifiedReceivableReceipt = (
  payment,
  isRTL = false,
  formatAmount,
  convertAmount,
  ...
  const printContent = `
    ...
    <html dir="${isRTL ? "rtl" : "ltr"}">
      <head>
        <meta charset="UTF-8">
        <title>Receivable Receipt ${mainData.elementNumber}</title>
...
const generatePaymentHTML = (
  ...
  return `
    <div class="bg-white text-black ${mode === "print" ? "print-receipt" : "p-1"} mobile-responsive" dir="${isRTL ? "rtl" : "ltr"}">
      <!-- Company Header -->
        ...
        <div class="">
          <h2 class="text-xl sm:text-2xl font-bold text-gray-800 mb-2">
            ${isRTL ? "إيصـــال دفـــع" : "Receivable RECEIPT"}
          </h2>
      ...
      <div class="grid grid-cols-1 gap-6 mb-6 sm:gap-8 sm:mb-8 lg:grid-cols-2 receipt-details">
        ...
        <div>
          <h3 class="text-base sm:text-lg font-semibold text-gray-800 mb-3 flex items-center">
            ...
            ${isRTL ? "تم التحصيل بواسطة" : "Customer Details"}
          </h3>
          <div class="space-y-2 text-gray-600 text-sm sm:text-base">
            <p class="font-semibold text-gray-800 break-words">${JSON.parse(quotation.main).customer.name || "No Customer"}</p>
            <p class="font-semibold text-gray-800 break-words">${JSON.parse(quotation.main).customer.email || "No Customer"}</p>
    </p>

        <div>
          <h3 class="text-base sm:text-lg font-semibold text-gray-800 mb-3 flex items-center">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="w-4 h-4 sm:w-5 sm:h-5 mr-2 rtl:mr-0 rtl:ml-2"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
            ${isRTL ? "تم التحصيل بواسطة" : "Created By"}
          </h3>
          <div class="space-y-2 text-gray-600 text-sm sm:text-base">
            <p class="font-semibold text-gray-800 break-words">${JSON.parse(quotation.createdBy)?.name || "No Customer"}</p>
    </p>
      ...
      ? `
            <div class="flex justify-between items-start">
              <span class="flex-shrink-0">${isRTL ? "معرف المعاملة:" : "Description:"}</span>
              <span class="font-medium text-right break-words font-mono text-xs">${quotation?.description || "N/A"}</span>
            </div>
            `
      : ""
    }
